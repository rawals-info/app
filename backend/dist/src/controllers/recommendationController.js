...
exports.getSummary = exports.deleteRecommendation = exports.createRecommendation = exports.markAllAsRead = exports.recordAction = exports.dismissRecommendation = exports.markAsRead = exports.getRecommendationById = exports.getRecommendations = void 0;
...
function matchConditions(ruleCond, answers) {
...
const getSummary = async (req, res) => {
    ...
    try {
        ...
        const rules = await models_2.RecommendationRule.findAll({ where: { categorySlug } });
        ...
        let matchedRule = rules.find(r => matchConditions(r.conditions, answerMap));
        if (!matchedRule)
            matchedRule = rules[0]; // fallback
        if (req.user) {
            const userProfile = await UserProfile_1.default.findOne({ where: { authId: req.user.id } });
            if (userProfile) {
                await models_2.RecommendationResult.create({
