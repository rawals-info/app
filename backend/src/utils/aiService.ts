...
interface FoodAnalysis {
  ...
  diabeticConcerns: {
    highGlycemicFoods: string[];
    lowNutrientFoods: string[];
    mealBalance: 'high_carb' | 'balanced';
    sugarContent: 'high' | 'moderate';
  };
...
interface ExerciseAnalysis {
  suggestedExercises: ExerciseRecommendation[];
  intensity?: string;
  duration?: string;
  frequency?: string;
  warnings: string[];
...
const analyzeFoodIntake = async (foodItems: FoodItem[], userProfile: UserProfile): Promise<FoodAnalysis> => {
  try {
    ...
    let highGlycemicIndex: string[] = [];
    let lowNutrientDensity: string[] = [];
 ...
 */
const generateExerciseRecommendations = async (
  ...
  try {
    ...
    const recommendations: ExerciseAnalysis = {
      suggestedExercises: [],
    ...
    if (hasHighBloodSugar) {
      recommendations.suggestedExercises.push(
        { name: 'Walking', benefits: 'Gentle on joints, helps lower blood sugar', duration: '30 minutes' },
        { name: 'Cycling', benefits: 'Low-impact cardio, improves insulin sensitivity', duration: '20-30 minutes' }
      );
      
      recommendations.tips.push(
        'For elevated blood sugar, moderate exercise can help bring levels down',
        'Check your blood sugar before and after exercise to understand how your body responds'
      );
      
      recommendations.warnings.push(
        'If blood sugar is above 240 mg/dL, check for ketones before exercising'
      );
    } else {
      recommendations.suggestedExercises.push(
        { name: 'Strength Training', benefits: 'Builds muscle, improves insulin sensitivity', duration: '20-30 minutes' },
        { name: 'Swimming', benefits: 'Full-body workout, gentle on joints', duration: '30 minutes' },
        { name: 'Yoga', benefits: 'Reduces stress, improves flexibility and balance', duration: '30 minutes' }
      );
      
      recommendations.tips.push(
        'Mix cardio and strength training for optimal blood sugar management',
        'Even short exercise sessions of 10 minutes can provide benefits'
      );
    }
    ...
    if (exerciseTypes.size < 3 && recentExercise) {
      ...
      recommendations.suggestedExercises.push(
        { name: 'Rowing', benefits: 'Full-body workout, builds endurance and strength', duration: '20 minutes' },
        { name: 'Dancing', benefits: 'Fun cardio that improves coordination', duration: '30 minutes' }
      );
    }
 ...
 */
const analyzeBloodSugarPatterns = async (
